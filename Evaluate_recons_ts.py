...
for video in sorted(videos_list):
    video_name = video.split('/')[-1]
    labels_list = np.append(labels_list, labels[0][3+label_length:videos[video_name]['length']+label_length])
...
for k,(imgs) in enumerate(test_batch):
    ...
    if k == label_length-3*(video_num+1):
    ...
    mse_imgs = torch.mean(loss_func_mse((outputs[0]+1)/2, (imgs[0,9:]+1)/2)).item()
    mse_feas = compactness_loss.item()
    
    # Calculating the threshold for updating at the test time
    point_sc = point_score(outputs, imgs[:,9:])
