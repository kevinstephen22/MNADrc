...
parser.add_argument('--exp_dir', type=str, default='log_abbey', help='directory of log')
...
for epoch in range(args.epochs):
    ...
    for j,(imgs) in enumerate(train_batch):
        ...
        outputs, _, _, m_items, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, mem_dist_loss = model.forward(imgs[:,0:12], m_items, True)
        
        
        optimizer.zero_grad()
        loss_pixel = torch.mean(loss_func_mse(outputs, imgs[:,12:]))
        loss = loss_pixel + 0.01*args.loss_compact * compactness_loss + 0.01*args.loss_separate * separateness_loss
        ...
        mem_dist_loss.backward()
        wandb.log({"rc_loss": loss_pixel,"mem_dist_loss":mem_dist_loss,"Loss": loss, "Compactness loss": compactness_loss, "Separateness Loss": separateness_loss})
        optimizer.step()
    if(epoch%5==0):
        torch.save(model, os.path.join(log_dir, str(epoch)+'model.pth'))
        torch.save(m_items, os.path.join(log_dir, str(epoch)+'keys.pt'))    
    scheduler.step()
    epoch_end = time.time()
    print('----------------------------------------')
    print('Epoch:', epoch+1)
